			...
			world.each([&](Entity* entity) {
				...
				if (ImGui::CollapsingHeader(buffer))
				{
					...
					if (entity->has<Transform2D>())
					{
						...
						{
							ImGui::InputFloat2("Position", transform->model[2].data);
							vec2f size = vec2f(transform->model[0].x, transform->model[1].y);
							if (ImGui::InputFloat2("Size", size.data))
							{
					...
					if (entity->has<TileLayer>())
					{
						...
						if (ImGui::TreeNodeEx(buffer, ImGuiTreeNodeFlags_Bullet | ImGuiTreeNodeFlags_DefaultOpen))
						{
							...
							ImGui::InputFloat4("Color", layer->color.data);
					...
					if (entity->has<Text>())
					{
						...
						if (ImGui::TreeNodeEx(buffer, ImGuiTreeNodeFlags_Bullet | ImGuiTreeNodeFlags_DefaultOpen))
						{
							ImGui::InputFloat4("Color", text->color.data);
					...
					if (entity->has<Camera2D>())
					{
						...
						if (ImGui::TreeNodeEx(buffer, ImGuiTreeNodeFlags_Bullet | ImGuiTreeNodeFlags_DefaultOpen))
						{
							ImGui::InputFloat2("Position", camera->position.data);
							ImGui::InputFloat2("Viewport", camera->viewport.data);
